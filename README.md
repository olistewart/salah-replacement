...
Ibrahim Mbaye, Yan Diomande, Luca Koleosho, Ousmane Dembélé, Michael Olise,
Vinícius Júnior, and Raphinha -- see [Results](#results) below.
...
4. **Candidate pool** (`src/05`). Every attacking player across the Big-5
   leagues in the last two completed seasons, no minutes threshold.
...
7. **Visuals** (`src/08`-`src/11`). PCA scatter (static + interactive), radar
...
- **No minutes threshold.** Every attacking player stays in the candidate
...
**Option 1: the notebook.** Open `notebooks/salah_replacement_analysis.ipynb`
...
Runs every numbered script in order, in one shared session (exactly like
running the notebook's cells top to bottom). Set `SALAH_DATA_DIR` as an
